...
df201906_sorted = df1906.sort_values(by=['LSOA code','Crime type'],ascending=True)
#print(df201906_sorted.head(10))


###### COUNT THE INSTANCES OF CRIME TYPE BY LOSA REGION ###########################################################################################
...
###### GET THHE TOTAL COUNT THE INSTANCES OF CRIME TYPE BY LOSA REGION ###########################################################################################
...
for val, row in dfcount1906.iterrows() :
    ...
    # creating a series on every itteration
    dfcount1906.loc[val, "Total"] = crime
    crime = row["Anti-social behaviour"] + row["Bicycle theft"] + row["Burglary"] + row["Criminal damage and arson"] + row["Drugs"] + row["Other crime"] + row["Other theft"] + row["Possession of weapons"] + row["Public order"] + row["Robbery"] + row["Shoplifting"] + row["Theft from the person"] + row["Vehicle crime"] +row["Violence and sexual offences"]
#    print(crime)
#    tc = crime + tc
